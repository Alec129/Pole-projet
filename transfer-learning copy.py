...
X_real_test_normalize  = normalize(X_real_test)
...
def train_one_epoch(epoch_index, tb_writer,loader):
...
training_real_loader = DataLoader(train_real_dataset, batch_size=batch, shuffle=True)
validation_real_loader = DataLoader(val_real_dataset, batch_size=batch, shuffle=False)


# We use the fonction loss CrossEntropy 
loss_fn = torch.nn.CrossEntropyLoss()
#optimizer = torch.optim.SGD(model.parameters(), lr=0.001, momentum=0.9)
optimizer = torch.optim.Adam(model.parameters(), lr=0.0001) # lr is reduce to have less impact on the model
...
for epoch in range(EPOCHS):
    ...
    # Make sure gradient tracking is on, and do a pass over the data
    model.train(True)
    avg_loss = train_one_epoch(epoch_number, writer,training_real_loader)
